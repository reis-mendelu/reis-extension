...
describe('resolveAscCredentials', () => {
  it('says which variable is missing instead of failing at upload time', () => {
    expect(() => resolveAscCredentials({ ASC_ISSUER_ID: 'i' })).toThrow(/ASC_KEY_ID/);
    expect(() => resolveAscCredentials({ ASC_KEY_ID: 'k' })).toThrow(/ASC_ISSUER_ID/);
  });

  it('lists every path it searched when the .p8 is absent', () => {
    expect(() => resolveAscCredentials({ ASC_KEY_ID: 'ABC123', ASC_ISSUER_ID: 'i' })).toThrow(
      /AuthKey_ABC123\.p8/
    );
  });
