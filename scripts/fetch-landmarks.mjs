...
const LANDMARKS = [
  {
    id: 1569,
    name: 'Koleje JAK Blok A',
    type: 'dormitory',
    url: 'http://skm.mendelu.cz/ubytovani/ubytovaci-zarizeni/27982-koleje-jak',
    phone: '+420 545 128 340',
    email: 'skm.koleje.jak@mendelu.cz',
    lon: 16.630584,
    lat: 49.216233,
  },
  {
    id: 1585,
    name: 'Koleje JAK Blok B',
    type: 'dormitory',
    url: 'http://skm.mendelu.cz/ubytovani/ubytovaci-zarizeni/27982-koleje-jak',
    phone: '+420 545 128 340',
    email: 'skm.koleje.jak@mendelu.cz',
  ...
  },
  {
    ...
    type: 'dormitory',
    url: 'http://skm.mendelu.cz/ubytovani/ubytovaci-zarizeni/27988-tauferovy-koleje',
    phone: '+420 541 422 051',
    email: 'jitka.chebenova@mendelu.cz',
    lon: 16.588826,
    lat: 49.214599,
  },
  {
    ...
    name: 'Kolej Akademie',
    type: 'dormitory',
    url: 'http://skm.mendelu.cz/ubytovani/ubytovaci-zarizeni/27987-kolej-akademie',
    phone: '+420 545 136 117',
    email: 'skm.koleje.akademie@mendelu.cz',
    lon: 16.614198,
    lat: 49.218173,
  },
  {
    id: 1587,
    name: 'Fakulta regionálního rozvoje a mezinárodních studií (FRRMS)',
    type: 'building',
    url: 'http://frrms.mendelu.cz',
    phone: '+420 545 136 306',
    email: 'info.frrms@mendelu.cz',
    lon: 16.614118,
    lat: 49.218161,
  },
  {
    id: 1623,
    name: 'Centrum sportovních aktivit MENDELU',
    type: 'building',
    url: 'http://csa.mendelu.cz',
    phone: '+420 541 212 698',
    email: 'jakub.havel@mendelu.cz',
    lon: 16.588579,
    lat: 49.21517,
  },
];
...
async function fetchFootprint(lm) {
  ...
  if (ways.length === 0) {
    console.warn(`MISS: no building way near ${lm.id} ${lm.name} — hand-check`);
    return null;
  }
  let best = null,
    bestD = Infinity;
  ...
  if (best && (best[0][0] !== best[best.length - 1][0] || best[0][1] !== best[best.length - 1][1]))
    best.push(best[0]);
  return best;
}
...
for (const lm of LANDMARKS) {
  ...
  try {
    ...
    console.warn(`ERROR ${lm.id}: ${e.message}`);
  }
  if (!ring) {
    console.warn(`SKIPPED ${lm.id} ${lm.name} — no footprint, fill in by hand`);
    continue;
  }
  out.push({
    id: lm.id,
