...
async function globalSetup(config: FullConfig) {
  ...
  if (!username || !password) {
    ...
    fs.writeFileSync(path.join(__dirname, '..', 'storageState.json'), JSON.stringify(emptyState));
    return;
  }
    ...
    if (!page.url().includes('login.pl')) {
      ...
      if (url.includes('/auth/')) {
        ...
        try {
          ...
          if (
            content.includes('Špatné jméno nebo heslo') ||
            content.includes('Neplatné přihlášení')
          ) {
