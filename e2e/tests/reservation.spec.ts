...
test.describe('Reservation Flow', () => {
  test('should book Individuální studovna 2 for next Friday 9:00-11:00', async ({
    extensionPage,
  }) => {
    ...
    const specificRoomSelect = extensionPage
      .locator('div')
      .filter({ hasText: 'Konkrétní místnost' })
      .last()
      .locator('select');
    ...
    const dayBtn = extensionPage
      .locator('.rdp-day')
      .filter({ hasText: dayNumber, hasNotText: '1' + dayNumber })
      .filter({ hasNotText: '2' + dayNumber })
      .filter({ hasNotText: '3' + dayNumber })
      .first();
    ...
    try {
      ...
      if (await errorToast.isVisible()) {
        ...
        const errorDesc = await extensionPage
          .locator('.sonner-toast-description')
          .innerText()
          .catch(() => '');
      ...
      throw new Error(
        `Reservation timeout. No toast appeared. Browser console errors: TODO: check test output.`
      );
    }
  });
});
