  ...
  test('inspect assessment tables', async ({ page }) => {
    ...
    for (const subject of SUBJECTS) {
      ...
      if (page.url().includes('login.pl')) {
        console.log(
          '❌ Redirected to login. Please ensure MENDELU_USER and MENDELU_PASS are set in .env'
        );
        return;
      }
      ...
      if ((await table.count()) === 0) {
        ...
        const allTables = await page
          .locator('table')
          .evaluateAll((tables) => tables.map((t) => ({ id: t.id, class: t.className })));
        ...
        const screenshotPath = path.join(
          ...
          '..',
          'test-results',
          `missing-table-${subject.code}.png`
        );
      ...
      if (rowCount > 0) {
        ...
        if (anyRows > 1) {
          const firstDataRow = await table
            .locator('tr')
            .nth(1)
            .locator('td')
            .evaluateAll((tds) => tds.map((td) => td.textContent?.trim() || ''));
