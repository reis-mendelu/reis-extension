...
test.describe('Student Subject Data Scraper', () => {
  ...
  const SCRAPE_FACULTY = process.env.SCRAPE_FACULTY
    ? parseInt(process.env.SCRAPE_FACULTY, 10)
    : null;
  ...
  test('scrape subject statistics', async ({ page }) => {
    ...
    console.log(
      `🚀 Starting scraper (${FACULTY_IDS.length} faculties, last ${MAX_SEMESTERS} semesters${SCRAPE_ALL_COURSES ? ', ALL courses' : ''})...`
    );
    ...
    for (const facultyId of FACULTY_IDS) {
      ...
      await page.goto(
        `https://is.mendelu.cz/auth/student/hodnoceni.pl?fakulta=${facultyId};lang=cz`
      );
      ...
      if ((await semesterTable.count()) === 0) {
        ...
        if (facultyId === FACULTY_IDS[0]) {
          // Only dump first failure to avoid log bloat
      ...
      const semesterHeader = semesterTable
        .locator('tr.zahlavi th')
        .filter({ hasText: 'Název období' });
      ...
      console.log(
        `   Found ${semesterLinks.length} semesters, processing ${limitedSemesterLinks.length}.`
      );
      ...
      for (const semester of limitedSemesterLinks) {
        ...
        for (let j = 0; j < courseCount; j++) {
          ...
          for (let c = 0; c < cellCount; c++) {
            ...
            const links = cells
              .nth(c)
              .locator('a[href*="hodnoceni.pl"][href*="predmet="][href*="obdobi="]');
        ...
        for (const course of courseLinks) {
          ...
          const chartRegex =
            /d3\.select\('#graph_\d+ svg'\)\s*\.datum\(\[\s*\{\s*values:\s*\[\s*([\s\S]*?)\]\s*}\s*,?\s*\]\)/;
          ...
          if (match && match[1]) {
            ...
            while ((vm = valueRegex.exec(valuesStr)) !== null) {
              ...
              if (grade === 'A') grades.A = count;
                ...
                grade === 'zk-nedost' ||
                grade === 'FN' ||
                grade.toLowerCase().includes('nedost')
              )
                grades.FN = count;
            ...
            console.log(
              `       📊 Final Outcome (Unique Students): Pass=${totalPass}, Fail=${totalFail}, N=${uniqueStudents}`
            );
          } else {
