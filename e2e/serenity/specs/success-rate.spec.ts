  ...
  it('visual audit', async () => {
    ...
    await Charlie.attemptsTo(
      ...
      Interaction.where('#actor injects mock data', async (actor) => {
        const page = await (
          BrowseTheWebWithPlaywright.as(actor) as unknown as {
            currentPage: () => Promise<{
              nativePage: () => Promise<{
                evaluate: (
                  fn: (args: { stats: MockStats; schedule: MockScheduleItem[] }) => void,
                  args: { stats: MockStats; schedule: MockScheduleItem[] }
                ) => Promise<void>;
              }>;
            }>;
          }
        ...
        await nativePage.evaluate(
          ({ stats, schedule }: { stats: MockStats; schedule: MockScheduleItem[] }) => {
            localStorage.setItem(
              'reis_success_rates',
              JSON.stringify({ data: stats, lastUpdated: new Date().toISOString() })
            );
      ...
      Interaction.where('#actor takes screenshots', async (actor) => {
        ...
        await nativePage.screenshot({
          path: path.join(artifactsPath, 'success_rate_light.png'),
          fullPage: true,
        });
        ...
        await nativePage.screenshot({
          path: path.join(artifactsPath, 'success_rate_dark.png'),
          fullPage: true,
        });
      })
    );
