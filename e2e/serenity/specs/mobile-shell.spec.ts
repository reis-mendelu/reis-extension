...
interface SeedStudyPlan {
    ...
    blocks: { title: string; groups: { name: string; statusDescription: string; subjects: SeedSubjectStatus[] }[] }[];
}
...
const SEEDED_PLAN: SeedStudyPlan = {
    ...
    creditsAcquired: 30,
    creditsRequired: 180,
    blocks: [{ title: '1. semestr', groups: [{ name: 'Povinné', statusDescription: '', subjects: [SEEDED_SUBJECT] }] }],
};
...
async function seedIndexedDbValue(page: Page, store: string, key: string, value: unknown): Promise<void> {
    await page.evaluate(
        ({ store, key, value }) =>
...
test.describe('mobile shell', () => {
    ...
    test('tapping a subject opens the drawer sheet and its tabs switch', async ({ page }) => {
        ...
        await bottomNav(page).getByRole('button', { name: NAV.subjects }).click();
        const subjectRow = page.getByTestId('subjects-screen').getByRole('button', { name: /E2E Algorithms/ });
    ...
    test('opening Eduroam from Student shows its sheet, and the backdrop dismisses it', async ({ page }) => {
        await gotoMobile(page);
