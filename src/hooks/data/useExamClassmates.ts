...
export function useExamClassmates(terminId: string | undefined): UseExamClassmatesResult {
  const classmates = useAppStore((state) =>
    terminId ? state.examClassmates[terminId] : undefined
  );
  const isTerminLoading = useAppStore((state) =>
    terminId ? !!state.examClassmatesLoading[terminId] : false
  );
  const error = useAppStore((state) =>
    terminId ? state.examClassmatesError[terminId] : undefined
  );
