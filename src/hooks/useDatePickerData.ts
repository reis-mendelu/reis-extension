  ...
  const availableDates = useMemo(() => {
    ...
    terms.forEach((t) => {
      if (!seen.has(t.date)) {
        ...
        dates.push({
          date: new Date(y ?? NaN, (m ?? NaN) - 1, d ?? NaN),
          dateStr: t.date,
          isFull: terms.filter((x) => x.date === t.date).every((x) => x.full),
        });
      }
    });
    return dates;
