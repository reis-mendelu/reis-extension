...
interface SyncedData {
  ...
  syllabuses?: Record<
    string,
    SyllabusRequirements | { cz: SyllabusRequirements; en: SyllabusRequirements }
  >;
...
export function useAppLogic() {
  ...
  useEffect(() => {
    if (!isInitialLoad.current) IndexedDBService.set('meta', 'reis_current_view', currentView);
  }, [currentView]);
  ...
  useEffect(() => {
    ...
    const handle = async (e: MessageEvent) => {
      ...
      const r =
        d.type === 'REIS_SYNC_UPDATE'
          ? (d.data as unknown as SyncedData)
          : d.type === 'REIS_DATA' && d.dataType === 'all'
            ? (d.data as unknown as SyncedData)
            : null;
      if (!r) return;
      ...
      try {
        ...
        if (r.odevzdavarny?.length) {
          ...
          if (userParams?.studium && userParams?.obdobi) {
            await IndexedDBService.set(
              'odevzdavarny',
              `${userParams.studium}_${userParams.obdobi}`,
              r.odevzdavarny
            );
        ...
        if (r.subjects?.data) {
          const existing = (await IndexedDBService.get('subjects', 'current')) as {
            data: Record<string, { nameCs?: string; nameEn?: string }>;
          } | null;
          if (existing?.data) {
            Object.keys(r.subjects.data).forEach((code) => {
              const incomingSub = r.subjects!.data[code]!; // safe: code came from Object.keys(r.subjects.data)
              const existingSub = existing.data[code];
              if (existingSub) {
                if (!incomingSub.nameCs && existingSub.nameCs)
                  incomingSub.nameCs = existingSub.nameCs;
                if (!incomingSub.nameEn && existingSub.nameEn)
                  incomingSub.nameEn = existingSub.nameEn;
              }
            });
          }
        ...
        if (r.files) {
          ...
          const toWrite = fileEntries
            .map(([c, f], i) => {
              ...
              if (
                !existing ||
                (f as Record<string, unknown>).cz ||
                !(existing as Record<string, unknown>).cz
              ) {
                return [c, f] as const;
              }
              return null;
            ...
            .filter(
              (e): e is readonly [string, ParsedFile[] | { cz: ParsedFile[]; en: ParsedFile[] }] =>
                e !== null
            );
        ...
        if (r.zaznamnik) {
          ...
          const persistEntries = Object.entries(r.zaznamnik).filter(
            ([, v]) => v && (v.ph.sections.length > 0 || v.vt.tests.length > 0)
          );
  ...
  const handleOpenSubjectFromSearch = (
    courseCode: string,
    courseName?: string,
    courseId?: string,
    facultyCode?: string,
