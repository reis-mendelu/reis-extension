...
export function useDragSelection(options: UseDragSelectionOptions = {}) {
  ...
  const selectionStartRef = useRef<Position | null>(null),
    initialSelectedIds = useRef<string[]>([]);
  const lastMousePos = useRef<Position | null>(null),
    ignoreClickRef = useRef(false);

  const process = useSelectionProcessor(
    selectionStartRef,
    containerRef,
    contentRef,
    itemRefs,
  ...
  const onMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (!isDraggingRef.current && selectionStartRef.current && containerRef.current) {
        ...
        const dx =
          e.clientX - rect.left + containerRef.current.scrollLeft - selectionStartRef.current.x;
        const dy =
          e.clientY - rect.top + containerRef.current.scrollTop - selectionStartRef.current.y;
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      if (e.button !== 0 || (e.target as HTMLElement).closest('button, a, input, [data-no-drag]'))
        return;
  ...
  return {
    ...
    handleMouseDown,
    ignoreClickRef,
    handleBackdropClick: (e: React.MouseEvent, cb: () => void) =>
      !ignoreClickRef.current && e.target === e.currentTarget && cb(),
