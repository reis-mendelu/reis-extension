...
function extractArchName(el: Element): string | null {
  ...
  const bold =
    Array.from(el.children).find((c) => c.tagName === 'B' || c.tagName === 'STRONG') ?? null;
...
function isEmptyState(el: Element): boolean {
  ...
  if (el.tagName === 'B' || el.tagName === 'STRONG') {
    ...
    return [s1?.textContent ?? '', s2?.textContent ?? '']
      .join(' ')
      .replace(/ /g, ' ')
      .includes('nemáte dosud');
  }
  return false;
}
...
export function parseVtPage(html: string): SubjectVt {
  ...
  const table = allTables.find((t) =>
    (t.querySelector('thead') ?? t.querySelector('tr.zahlavi'))?.textContent?.includes(
      'Název testu'
    )
  );
