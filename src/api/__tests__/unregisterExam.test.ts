...
describe('unregisterExam verification', () => {
  ...
  it('returns success when the unregister link for the term is gone afterward', async () => {
    vi.mocked(client.fetchWithAuth).mockResolvedValue(
      htmlResponse('<a href="terminy_seznam.pl?termin=555;prihlasit_ihned=1">register</a>')
    );
  ...
  it('returns failure when the term is still shown as registered afterward', async () => {
    vi.mocked(client.fetchWithAuth).mockResolvedValue(
      htmlResponse('<a href="terminy_seznam.pl?termin=555;odhlasit_ihned=1">unregister</a>')
    );
    const res = await unregisterExam('555');
  ...
  it('returns failure with error when studium params are missing', async () => {
    vi.mocked(userParams.getUserParams).mockResolvedValue(
      null as Awaited<ReturnType<typeof userParams.getUserParams>>
    );
    const res = await unregisterExam('555');
    expect(res.success).toBe(false);
  });
