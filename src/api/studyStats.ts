...
export async function fetchStudyStats(studium: string, obdobi: string): Promise<StudyStats | null> {
  try {
    const res = await fetchWithAuth(
      `${STATS_URL}?vyber=stat_info;studium=${studium};obdobi=${obdobi};lang=cz`
    );
...
function extractSemesterStats(data: Record<string, string>): SemesterStats {
  return {
    ...
    completedSubjects: parseNumber(
      data['Počet odstudovaných předmětů za dané studijní období'] || '0'
    ),
    gpa: parseNumber(data['Průměr z odstudovaných předmětů za dané studijní období'] || '0'),
    gpaWithFails: parseNumber(
      data[
        'Průměr z odstudovaných předmětů za dané studijní období (včetně neúspěšně ukončených předmětů)'
      ] || '0'
    ),
  };
}
...
export function parseStudyStats(doc: Document): StudyStats | null {
  ...
  const semesterTables = tables.filter((t) =>
    (t.textContent || '').includes('kreditů za dané studijní období')
  );
  ...
  const previousData =
    semesterTables.length > 1 ? parseStatsTable(semesterTables[1] as HTMLTableElement) : null;
  ...
  return {
    ...
    creditsLastTwoPeriods: parseNumber(
      totalData['Počet získaných kreditů za poslední dvě období'] || '0'
    ),
    repeatedSubjects: parseNumber(totalData['Počet opakovaných předmětů za celé studium'] || '0'),
    registrationVouchersInitial: parseNumber(
      totalData['Počáteční stav registračních poukázek'] || '0'
    ),
    registrationVouchersCurrent: parseNumber(
      totalData['Aktuální stav registračních poukázek'] || '0'
    ),
    gpaTotal: parseNumber(totalData['Průměr z odstudovaných předmětů za celé studium'] || '0'),
    weightedGpaTotal: parseNumber(
      totalData[
        'Vážený průměr z odstudovaných předmětů za celé studium (včetně neúspěšně ukončených předmětů)'
      ] || '0'
    ),
  };
}
