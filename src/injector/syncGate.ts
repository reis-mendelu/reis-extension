...
async function runUnderLock(reason: SyncReason): Promise<boolean> {
  ...
  if (!locks) return run(); // no Web Locks (older WebView, tests) — nothing to coordinate
  ...
  try {
    await locks.request(SYNC_LOCK_NAME, { signal: controller.signal }, run);
    return true;
  } catch {
    return run();
  } finally {
...
export async function requestSync(reason: SyncReason): Promise<boolean> {
  ...
  if (AUTOMATIC.has(reason) && Date.now() - lastRunAt < MIN_SYNC_GAP) return false;
