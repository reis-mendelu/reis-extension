...
export const createSubjectsSlice: AppSlice<SubjectsSlice> = (set, get) => ({
  ...
  fetchSubjects: async () => {
    ...
    try {
      ...
      const codes = Object.keys((data as { data?: Record<string, unknown> } | null)?.data ?? {});
      if (codes.length > 0)
        void get()
  ...
  setCourseNickname: (courseCode, nickname) => {
    ...
    IndexedDBService.set('meta', 'course_nicknames', newNicknames).catch((e) =>
      logError('SubjectsSlice.setCourseNickname', e)
    );
  },
  setCourseDeadlines: (courseCode, deadlines) => {
    ...
    IndexedDBService.set('meta', 'course_deadlines', newDeadlines).catch((e) =>
      logError('SubjectsSlice.setCourseDeadlines', e)
    );
  },
});
