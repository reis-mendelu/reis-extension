...
describe('createSuggestionsSlice', () => {
  ...
  it('loadSuggestions preserves existing state when the read fails', async () => {
    // A failed read is ambiguous with a genuinely empty inbox unless the API
    // distinguishes them. listSuggestions() returns null on failure; the
    // slice must not treat that as "nothing to triage".
    listSuggestions.mockResolvedValue([row(1, 'new'), row(2, 'done')]);
    await state.loadSuggestions();
    expect(state.suggestions).toHaveLength(2);

    listSuggestions.mockResolvedValue(null);
    await state.loadSuggestions();

    expect(state.suggestions).toHaveLength(2);
    expect(state.suggestions.map((s) => s.id)).toEqual([1, 2]);
    expect(state.suggestionsUnread).toBe(1);
  });

  it('loadSuggestions sets empty state when the read genuinely yields no rows', async () => {
    listSuggestions.mockResolvedValue([row(1, 'new')]);
    await state.loadSuggestions();
  ...
  it('a failed status write whose resync read also fails leaves the previously-loaded inbox visible', async () => {
    // The motivating scenario: one flaky connection (or an expired admin
    // session) can plausibly fail both the write and its resync read. That
    // must not blank the inbox and hide rows that had loaded fine moments
    // earlier.
    listSuggestions.mockResolvedValue([row(1, 'new'), row(2, 'new')]);
    await state.loadSuggestions();
    expect(state.suggestions).toHaveLength(2);
    ...
    setSuggestionStatus.mockResolvedValue(false);
    listSuggestions.mockResolvedValue(null); // resync read also fails

    await state.updateSuggestionStatus(1, 'done');

    // The optimistic write is still unconfirmed (the server write failed and
    // the resync couldn't correct it either), but the inbox itself must not
    // be wiped out — both rows stay visible rather than reading as empty.
    expect(state.suggestions).toHaveLength(2);
    expect(state.suggestions.map((s) => s.id).sort()).toEqual([1, 2]);
    expect(state.suggestionsUnread).toBe(1);
  });
});
