...
describe('createFeedbackSlice', () => {
  ...
  it('submitNps should call API and persist to IndexedDB', async () => {
    ...
    // No student identifier: the call carries only the rating itself.
    expect(submitFeedback).toHaveBeenCalledWith('nps', '4', '2026S', undefined);
  ...
  it('submitNps should pass the reason through to submitFeedback when provided', async () => {
    ...
    expect(submitFeedback).toHaveBeenCalledWith('nps', '2', '2026S', 'bug');
  });
