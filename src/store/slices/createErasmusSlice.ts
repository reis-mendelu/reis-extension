...
import {
  fetchErasmusReports,
  getStoredErasmusData,
  fetchErasmusConfig as fetchErasmusConfigApi,
} from '../../api/erasmus';
...
export const createErasmusSlice: AppSlice<ErasmusSlice> = (set, get) => ({
  ...
  fetchUniversities: async (alpha2: string) => {
    ...
    try {
      ...
      const cached = (await IndexedDBService.get(
        'meta',
        `${UNIVERSITIES_KEY_PREFIX}:${alpha2}`
      )) as University[] | null;
      ...
      loggers.ui.debug(
        `[fetchUniversities] response keys:`,
        json && typeof json === 'object' ? Object.keys(json) : typeof json
      );
      type HeiAttrs = {
        name?: { lang: string; string: string }[];
        other_id?: { type: string; value: string }[];
        city?: string;
        website_url?: string;
      };
      type HeiItem = { attributes?: HeiAttrs };
      const items: HeiItem[] = Array.isArray((json as { data?: unknown })?.data)
        ? (json as { data: HeiItem[] }).data
        : [];
      const parsed: University[] = items
        .map((item) => {
  ...
  reorderErasmusTableBCourse: (optionId: string, fromIndex: number, toIndex: number) => {
    ...
    if (!moved) return; // fromIndex out of range — nothing to move
    current.splice(toIndex, 0, moved);
    const next = { ...all, [optionId]: current };
    set({ erasmusTableBCourses: next });
    IndexedDBService.set('meta', TABLE_B_COURSES_KEY, next).catch((e) =>
      logError('ErasmusSlice.reorderErasmusTableBCourse', e)
    );
  },
  toggleErasmusTableBCourse: (optionId: string, code: string) => {
    ...
    const next = {
      ...all,
      [optionId]: isRemoving ? current.filter((c) => c !== code) : [...current, code],
    };
    set({ erasmusTableBCourses: next });
    IndexedDBService.set('meta', TABLE_B_COURSES_KEY, next).catch((e) =>
      logError('ErasmusSlice.toggleErasmusTableBCourse', e)
    );
    if (isRemoving) {
      ...
      IndexedDBService.set('meta', VERDICTS_KEY, verdicts).catch((e) =>
        logError('ErasmusSlice.toggleErasmusTableBCourse:verdicts', e)
      );
      IndexedDBService.set('meta', AI_RESULTS_KEY, aiResults).catch((e) =>
        logError('ErasmusSlice.toggleErasmusTableBCourse:aiResults', e)
      );
    }
  },
  setErasmusStudentInfo: (data: Partial<ErasmusStudentInfo>) => {
    ...
    IndexedDBService.set('meta', STUDENT_INFO_KEY, next).catch((e) =>
      logError('ErasmusSlice.setErasmusStudentInfo', e)
    );
  },
  initErasmusStudentInfo: (params: {
    ...
    const allBlank =
      !current.firstName && !current.lastName && !current.studyCode && !current.studentId;
    if (!allBlank) return;
    const fullName = params.fullName?.trim() ?? '';
    const parts = fullName.split(' ');
    const lastName = parts.length > 1 ? parts[parts.length - 1]! : fullName; // safe: length checked above
    ...
    const studyCode =
      programParts.length >= 2 ? `${programParts[0]}-${programParts[1]}` : rawProgram;
  ...
  setErasmusVerdict: (code: string, verdict: 'approved' | 'rejected' | null) => {
    ...
    IndexedDBService.set('meta', VERDICTS_KEY, verdicts).catch((e) =>
      logError('ErasmusSlice.setErasmusVerdict', e)
    );
  },
  setErasmusAiResult: (code: string, result: AIComparisonResult | null) => {
    ...
    IndexedDBService.set('meta', AI_RESULTS_KEY, next).catch((e) =>
      logError('ErasmusSlice.setErasmusAiResult', e)
    );
  },
  setErasmusPdfAssignment: (courseCode: string, filename: string | null) => {
    ...
    IndexedDBService.set('meta', PDF_ASSIGNMENTS_KEY, assignments).catch((e) =>
      logError('ErasmusSlice.setErasmusPdfAssignment', e)
    );
  },
  pinErasmusUniversity: (name: string) => {
    ...
    IndexedDBService.set('meta', PINNED_UNIVERSITIES_KEY, next).catch((e) =>
      logError('ErasmusSlice.pinErasmusUniversity', e)
    );
  },
  unpinErasmusUniversity: (name: string) => {
    ...
    set({ erasmusPinnedUniversities: next });
    IndexedDBService.set('meta', PINNED_UNIVERSITIES_KEY, next).catch((e) =>
      logError('ErasmusSlice.unpinErasmusUniversity', e)
    );
  },
  addErasmusUploadedPdf: (filename: string, text: string, base64: string) => {
    ...
    IndexedDBService.set('meta', UPLOADED_PDFS_KEY, next).catch((e) =>
      logError('ErasmusSlice.addErasmusUploadedPdf', e)
    );
  },
  removeErasmusUploadedPdf: (filename: string) => {
    ...
    set({ erasmusUploadedPdfs: next });
    IndexedDBService.set('meta', UPLOADED_PDFS_KEY, next).catch((e) =>
      logError('ErasmusSlice.removeErasmusUploadedPdf', e)
    );
  },
  clearErasmusUploadedPdfs: () => {
    set({ erasmusUploadedPdfs: {} });
    IndexedDBService.set('meta', UPLOADED_PDFS_KEY, {}).catch((e) =>
      logError('ErasmusSlice.clearErasmusUploadedPdfs', e)
    );
  },
  addErasmusTableAOption: () => {
    ...
    IndexedDBService.set('meta', TABLE_A_OPTIONS_KEY, next).catch((e) =>
      logError('ErasmusSlice.addErasmusTableAOption', e)
    );
  },
  removeErasmusTableAOption: (id: string) => {
    ...
    set({ erasmusTableAOptions: next });
    IndexedDBService.set('meta', TABLE_A_OPTIONS_KEY, next).catch((e) =>
      logError('ErasmusSlice.removeErasmusTableAOption', e)
    );
  },
  updateErasmusTableAOptionHeader: (id, data) => {
    ...
    set({ erasmusTableAOptions: next });
    IndexedDBService.set('meta', TABLE_A_OPTIONS_KEY, next).catch((e) =>
      logError('ErasmusSlice.updateErasmusTableAOptionHeader', e)
    );
  },
  addErasmusTableACourse: (
    ...
    const next = get().erasmusTableAOptions.map((o) =>
      o.id === optionId ? { ...o, courses: [...o.courses, course] } : o
    );
    set({ erasmusTableAOptions: next });
    IndexedDBService.set('meta', TABLE_A_OPTIONS_KEY, next).catch((e) =>
      logError('ErasmusSlice.addErasmusTableACourse', e)
    );
  },
  removeErasmusTableACourse: (optionId: string, index: number) => {
    const next = get().erasmusTableAOptions.map((o) =>
      o.id === optionId ? { ...o, courses: o.courses.filter((_, i) => i !== index) } : o
    );
    set({ erasmusTableAOptions: next });
    IndexedDBService.set('meta', TABLE_A_OPTIONS_KEY, next).catch((e) =>
      logError('ErasmusSlice.removeErasmusTableACourse', e)
    );
  },
  addErasmusTableBManualCourse: (
    ...
    IndexedDBService.set('meta', TABLE_B_MANUAL_KEY, next).catch((e) =>
      logError('ErasmusSlice.addErasmusTableBManualCourse', e)
    );
  },
  removeErasmusTableBManualCourse: (optionId: string, index: number) => {
    ...
    set({ erasmusTableBManualCourses: next });
    IndexedDBService.set('meta', TABLE_B_MANUAL_KEY, next).catch((e) =>
      logError('ErasmusSlice.removeErasmusTableBManualCourse', e)
    );
  },
  reorderErasmusTableBManualCourse: (optionId: string, fromIndex: number, toIndex: number) => {
    ...
    set({ erasmusTableBManualCourses: next });
    IndexedDBService.set('meta', TABLE_B_MANUAL_KEY, next).catch((e) =>
      logError('ErasmusSlice.reorderErasmusTableBManualCourse', e)
    );
  },
  reorderErasmusTableACourse: (optionId: string, fromIndex: number, toIndex: number) => {
    const next = get().erasmusTableAOptions.map((o) => {
      ...
      if (!moved) return o; // fromIndex out of range — nothing to move
      courses.splice(toIndex, 0, moved);
      return { ...o, courses };
    });
    set({ erasmusTableAOptions: next });
    IndexedDBService.set('meta', TABLE_A_OPTIONS_KEY, next).catch((e) =>
      logError('ErasmusSlice.reorderErasmusTableACourse', e)
    );
  },
  loadErasmusState: async () => {
    try {
      const tableBCourses = (await IndexedDBService.get('meta', TABLE_B_COURSES_KEY)) as Record<
        string,
        string[]
      > | null;
      if (tableBCourses) set({ erasmusTableBCourses: tableBCourses });

      const tableBManual = (await IndexedDBService.get('meta', TABLE_B_MANUAL_KEY)) as Record<
        string,
        { code: string; name: string; credits: number }[]
      > | null;
      if (tableBManual) set({ erasmusTableBManualCourses: tableBManual });

      const studentInfo = (await IndexedDBService.get(
        'meta',
        STUDENT_INFO_KEY
      )) as ErasmusStudentInfo | null;
      if (studentInfo) set({ erasmusStudentInfo: { ...EMPTY_STUDENT_INFO, ...studentInfo } });

      const tableAOptions = (await IndexedDBService.get('meta', TABLE_A_OPTIONS_KEY)) as
        ErasmusUniversityOption[] | null;
      if (tableAOptions && tableAOptions.length > 0) {
        set({ erasmusTableAOptions: tableAOptions });
      } else {
        const oldTableA = (await IndexedDBService.get('meta', TABLE_A_COURSES_KEY)) as
          ErasmusUniversityOption['courses'] | null;
        if (oldTableA && oldTableA.length > 0) {
          ...
          IndexedDBService.set('meta', TABLE_A_OPTIONS_KEY, initial).catch((e) =>
            logError('ErasmusSlice.loadErasmusState:migrateTableA', e)
          );
        }
      }

      const verdicts = (await IndexedDBService.get('meta', VERDICTS_KEY)) as Record<
        string,
        'approved' | 'rejected'
      > | null;
      if (verdicts) set({ erasmusVerdicts: verdicts });

      const aiResults = (await IndexedDBService.get('meta', AI_RESULTS_KEY)) as Record<
        string,
        AIComparisonResult
      > | null;
      if (aiResults) set({ erasmusAiResults: aiResults });

      const assignments = (await IndexedDBService.get('meta', PDF_ASSIGNMENTS_KEY)) as Record<
        string,
        string
      > | null;
      if (assignments) set({ erasmusPdfAssignments: assignments });

      const pinned = (await IndexedDBService.get('meta', PINNED_UNIVERSITIES_KEY)) as
        string[] | null;
      if (pinned) set({ erasmusPinnedUniversities: pinned });

      const pdfs = (await IndexedDBService.get('meta', UPLOADED_PDFS_KEY)) as Record<
        string,
        { text: string; base64: string }
      > | null;
      if (pdfs) set({ erasmusUploadedPdfs: pdfs });

      const uiState = (await IndexedDBService.get('meta', ERASMUS_UI_STATE_KEY)) as {
        activeTab?: 'plan' | 'explore';
        planPhase?: 'select' | 'review';
      } | null;
      if (uiState) {
