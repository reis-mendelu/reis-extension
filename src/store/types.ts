...
export interface ExamSlice {
  ...
  /** `${terminId}:${lang}` → teacher's Poznámka. null = fetched-no-note. undefined = not fetched. In-memory only (session-bound TTL). */
  ...
  fetchExamNotePriority: (terminId: string, lang: Language) => Promise<void>;
}
