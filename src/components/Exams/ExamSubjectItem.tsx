      ...
      <AccordionHeader className="flex">
        <AccordionTrigger className="flex flex-1 items-center justify-between px-6 py-4 bg-base-100 hover:bg-base-200 group data-[state=open]:bg-base-200/50">
          <div className="flex flex-col items-start gap-0.5">
            <span className="font-semibold text-base-content text-base truncate max-w-[350px]">
              {displayName}
            </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <ChevronDown
              className="text-base-content/50 transition-transform duration-200 group-data-[state=open]:rotate-180"
              size={16}
            />
          </div>
        </AccordionTrigger>
        ...
        <div className="px-6 py-4 bg-base-200/50 space-y-2">
          {subject.sections.map((sec: ExamSection) => (
            <ExamSectionItem
              key={sec.id}
              section={sec}
              isProcessing={processingId === sec.id}
