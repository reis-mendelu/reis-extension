...
export function ExamSectionItem({
  ...
  onOpenPicker: (section: ExamSection, target: HTMLElement) => void;
}) {
  ...
  return (
    <div className="group flex items-center justify-between p-3 rounded-lg border border-transparent hover:bg-base-100 hover:shadow-sm hover:border-base-300 transition-all">
      <div className="flex flex-col gap-0.5 min-w-0">
        <span
          className="font-medium text-base-content text-sm capitalize truncate"
          title={section.name}
        >
          {section.name}
        </span>
        {section.status === 'registered' && section.registeredTerm && (
          <span className="text-xs text-base-content/60">
            {section.registeredTerm.date} {section.registeredTerm.time} (
            {getDOW(section.registeredTerm.date)})
      ...
      <div className="flex items-center gap-2 shrink-0">
        <Button
          variant={section.status === 'registered' ? 'ghost' : 'default'}
          size="sm"
          onClick={(e) => onOpenPicker(section, e.currentTarget)}
