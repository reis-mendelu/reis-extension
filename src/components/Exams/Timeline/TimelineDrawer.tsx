  ...
  return (
    <div className="border-t border-base-200 bg-base-50/40 px-4 py-3 animate-in slide-in-from-top-1 duration-150">
      <div className="flex items-center gap-3 flex-wrap">
        {deadline ? (
          <span
            className={`flex flex-wrap items-center gap-1.5 text-xs font-semibold ${urgency === 'expired' ? 'text-base-content/30' : urgencyText[urgency]} ${urgency === 'critical' ? 'animate-pulse' : ''}`}
          >
            <AlertCircle size={12} className="shrink-0" />
            <span className={urgency === 'expired' ? 'line-through' : ''}>
        ...
        {onUnregister && (
          <div
            className={
              urgency === 'expired' ? 'tooltip tooltip-left cursor-not-allowed ml-auto' : 'ml-auto'
            }
            data-tip={urgency === 'expired' ? t('exams.afterDeadlineCannotDeregister') : undefined}
          >
            <button
              onClick={() => {
                if (urgency !== 'expired') onUnregister(section);
              }}
