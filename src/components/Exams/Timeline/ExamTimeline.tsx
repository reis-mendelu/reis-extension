...
const ExamTimeline: React.FC<ExamTimelineProps> = ({
  ...
  const parseDateTime = (d: string, tStr: string) => {
    ...
    return new Date(
      ...
      +parts[1] - 1,
      +parts[0],
      ...(tStr.split(':').map(Number) as [number, number])
    );
  };
  ...
  if (!isHorizontal) {
    return (
      <div
        className={`p-8 max-w-2xl mx-auto bg-base-100 rounded-box shadow-sm border border-base-200 ${className}`}
      >
        ...
        <ul
          className={`timeline timeline-snap-icon max-md:timeline-compact timeline-vertical ${className}`}
        >
  ...
  const now = Date.now();
  const nextIndex = sortedExams.findIndex(
    (e) => parseDateTime(e.term.date, e.term.time).getTime() >= now
  );

  return (
    <div
      className={`overflow-x-auto w-full py-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${className}`}
    >
