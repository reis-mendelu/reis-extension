...
const getTranslatedLabel = (id: string, defaultLabel: string, lang: string, pages: PageCategory[] = pagesData) => {
...
export const mainItems = (sid: string, oid: string, t: (key: string) => string, lang: string = 'cz', pinnedPages: PinnedPage[] = [], navPages?: PageCategory[] | null): MenuItem[] => [
    ...
    { id: 'subjects', label: t('sidebar.subjects'), icon: <Book className="w-5 h-5" /> },
    {
        ...
        children: [
            ...
            ...pinnedPages.map(p => ({
                id: p.id,
                label: getTranslatedLabel(p.id, p.label, lang, navPages ?? pagesData),
