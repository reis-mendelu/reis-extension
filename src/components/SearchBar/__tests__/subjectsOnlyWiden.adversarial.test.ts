...
describe('useSearch — subjectsOnly survives widening to university scope', () => {
  ...
  it('THE critical case: people must not leak in after widenToUniversity when subjectsOnly is set', async () => {
    ...
    // waitFor on the RESULT, not just the call count: the mock resolving does
    // not mean React has flushed the state, so asserting immediately after read
    // `undefined` on slower CI runners. The post-widen assertion below already
    // waits this way; this one did not.
    await waitFor(() =>
    ...
    mockExecuteSearch.mockResolvedValueOnce({
      people: [
        ...
        person('p3', 'Eva Dvořáková'),
      ],
      subjects: [
        subj('1', 'EBC-ST', 'Statistika', 'LS 2025/2026'),
        subj('9', 'XYZ-ST', 'Statistika II', 'LS 2025/2026', 'AF'),
      ],
      subjectsTruncated: false,
    });
    act(() => {
  ...
  it('a fresh keystroke resets scope to faculty AND keeps people excluded under subjectsOnly', async () => {
    ...
    const { result, rerender } = renderHook(({ q }) => useSearch(q, true), {
      initialProps: { q: 'stat' },
    });
    await waitFor(() => expect(mockExecuteSearch).toHaveBeenCalledTimes(1));
  ...
  it('flipping subjectsOnly true->false on a live hook (defensive: prop should be respected per-render)', async () => {
    ...
    const { result, rerender } = renderHook(({ so }) => useSearch('statistika', so), {
      initialProps: { so: true },
    });
    await waitFor(() => expect(mockExecuteSearch).toHaveBeenCalledTimes(1));
    expect(result.current.sections.find((s) => s.key === 'people')).toBeUndefined();
    ...
    act(() => {
      result.current.widenToUniversity();
    }); // force a fresh fetch deterministically
    await waitFor(() => expect(mockExecuteSearch).toHaveBeenCalledTimes(2));
    await waitFor(() =>
      expect(result.current.sections.find((s) => s.key === 'people')?.results.length).toBe(1)
    );
  });
});
