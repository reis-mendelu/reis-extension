...
function TableBRow({ subject, onRemove }: {
    ...
    <div className="border-b border-base-300/50 last:border-b-0">
      <div className="grid grid-cols-[auto_1fr_auto_auto] gap-2 items-center px-3 py-2 text-xs hover:bg-base-200/30 transition-colors">
        ...
        <span className="tabular-nums font-bold text-base-content/70 w-12 text-right">
          {subject.credits >= 999 ? '?' : subject.credits}
        </span>
        <button
          onClick={onRemove}
...
export function LATableB({ plan, selectedCodes, onToggle }: Props) {
  ...
  const totalCredits = selected.reduce((sum, s) => sum + (s.credits < 999 ? s.credits : 0), 0);
