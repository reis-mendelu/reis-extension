...
export function ErasmusVerifyDot({ courseCode, optionId, rowIndex }: ErasmusVerifyDotProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 15 * 1024 * 1024) {
      alert('PDF is too large (max 15 MB).');
      return;
    }
  ...
  const runAICheck = useCallback(async () => {
    ...
    try {
      ...
      const primaryMetadata = {
        ...
        code: targetBCode,
        name:
          targetSyl?.courseInfo?.courseNameEn ?? targetSyl?.courseInfo?.courseNameCs ?? targetBCode,
      };
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-bold text-success">
                    {t('erasmus.verifyAutoApprovedPercent')}
                  </span>
                </div>
                ...
                {!isPastingSyllabus ? (
                  <>
                    ...
                    >
                      ...
                      {t('erasmus.verifyChoosePdf')}{' '}
                      <span className="text-base-content/40 font-normal ml-1">
                        {t('erasmus.verifyMax15Mb')}
                      </span>
                    </button>

                    {/* Stated before the file picker opens, not buried in a policy.
                  The intended upload is a foreign course syllabus — a public
                  document — but nothing stops a student reaching for a
                  transcript, so say plainly what leaves and where it goes. */}
                    <p className="px-1 text-[10px] leading-snug text-base-content/50">
                      {t('erasmus.verifyAiNotice')}
                    </p>

                    <div className="divider my-0 text-[10px] text-base-content/30 h-4 uppercase">
                      {t('erasmus.pasteOr')}
                    </div>
            ...
            {pdfData && !isAutoApproved && (
              <>
                ...
                {aiStatus === 'done' && aiResult?.verdict === 'approved' && (
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-1.5">
                      <CheckCircle size={13} className="text-success shrink-0" />
                      <span className="text-xs font-bold text-success">
                        {similarityPct}% {t('erasmus.verifyApproved')}
                      </span>
                    </div>
                    {aiResult.reasoning && (
                    ...
                    <div className="flex justify-end">
                      <button
                        onClick={runAICheck}
                        className="btn btn-ghost btn-xs text-[10px] gap-1 text-base-content/40"
                      >
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-bold text-error">
                        {similarityPct}% {t('erasmus.verifyRejected')}
                      </span>
                    </div>
                    {aiResult.reasoning && (
                      <p className="text-xs text-base-content/70 italic">"{aiResult.reasoning}"</p>
                ...
                {aiStatus === 'idle' && targetBCode && (
                  <button
                    className="btn btn-soft btn-primary btn-sm w-full text-xs font-bold gap-1.5 shadow-sm border border-primary/20"
                    onClick={runAICheck}
                  >
