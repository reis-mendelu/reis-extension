...
export function zameraniInsights(
  ...
  return plan.zameranis
    .map((z) => {
      const subjects = z.subjects.map((r) => {
        const credits = creditByCode.get(r.code) ?? 0;
        const stat = subjectStat(successRates[r.code]);
        // The plan first, then the success-rate record — the same order, and the
        // same guard, as `SubjectRow`.
        //
        // `idByCode` only covers the student's OWN plan blocks, and a zaměření
        // deliberately lists subjects they have not got: comparing zaměření
        // before picking one is the whole purpose of this card. Against real data
        // that left 10 of 20 rows with no id, and `ZameraniRow` sends an id-less
        // row to `onSearch` — which on the phone is the search SHEET. So tapping
        // "Enterprise aplikace" opened the people-and-subject search, reported as
        // "a completely different screen".
        //
        // `resolvePredmetId` still rejects the junk ids reis-data carries for the
        // mobility placeholders ("999"), so those keep the search fallback rather
        // than opening the wrong subject.
        const id = idByCode.get(r.code) || resolvePredmetId(successRates[r.code]) || '';
        return { code: r.code, id, name: nameByCode.get(r.code) ?? r.name, credits, stat };
      });
      let weightedNum = 0,
        weightedDen = 0,
        worst: { code: string; rate: number } | null = null,
        lowSample = 0;
      let totalCredits = 0;
      for (const s of subjects) {
        if (isRealCredits(s.credits)) totalCredits += s.credits;
        if (!s.stat || s.stat.n < MIN_SAMPLE) {
          lowSample++;
          continue;
        }
