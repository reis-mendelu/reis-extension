...
import { buildRegisteredExams, buildOpenExams, type RegisteredExam, type OpenExam } from '../../../utils/mobile/examRows';
...
function ClassmateLine({ termId, t, language }: {
    termId?: string;
    ...
    return (
        <span className="flex items-center gap-1.5 text-sm text-base-content/70">
            ...
            {classmates.length > 0
                ? t(`mobile.exams.mates${pluralSuffix(language, classmates.length)}`, { count: classmates.length })
...
export function ExamsScreen() {
    ...
    const { thisWeek, later } = useMemo(() => splitByWeek(registered, (r) => r.date, now), [registered, now]);
        ...
        <div data-testid="exams-screen" className="flex flex-1 flex-col overflow-hidden">
            ...
            {exams.length === 0 ? (
                ...
                <>
                    ...
                    />
                    <div data-testid="exam-list" className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 pb-24 pt-3">
                        {thisWeek.length > 0 && (
