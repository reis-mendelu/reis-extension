...
export function AverageAccordion({ studyStats, comparison }: AverageAccordionProps) {
    ...
    const hasAverages = !!studyStats
        && [studyStats.currentSemester.gpa, studyStats.gpaTotal, studyStats.weightedGpaTotal].some((v) => v > 0);
            ...
            {open && (
                <div className="flex flex-col px-4 pb-3.5">
                    {hasAverages && studyStats && (
                        <>
                            <AverageRow label={t('mobile.subjects.avgSemester')} value={formatGpa(studyStats.currentSemester.gpa)} />
                            <AverageRow label={t('mobile.subjects.avgTotal')} value={formatGpa(studyStats.gpaTotal)} />
                            <AverageRow label={t('mobile.subjects.avgWeighted')} value={formatGpa(studyStats.weightedGpaTotal)} />
                        </>
                    )}
                    {standing && comparison && (
