...
function StatusIcon({ status }: { status: DownloadStatus }) {
    if (status === 'loading') return <Loader2 className="h-4 w-4 flex-shrink-0 animate-spin text-base-content/50" />;
...
export function DocsSheet({ onClose }: DocsSheetProps) {
    ...
    return (
        <Sheet size="content" onClose={onClose}>
            <SheetHeader title={t('documents.title')} subtitle={t('documents.sheetSubtitle')} onClose={onClose} />
