...
export function useAutoRegistration() {
  ...
  const performRegisterHit = useCallback(
    async (section: ExamSection, termId: string) => {
      try {
  ...
  const fireBatteringRam = useCallback(
    (termId: string, section: ExamSection) => {
  ...
  const arm = useCallback(
    (term: ExamTerm, section: ExamSection) => {
      ...
      const existingSectionTermId = Array.from(armedTerms.entries()).find(
        ([, armed]) => armed.section.id === section.id
      )?.[0];
