...
export function TermsSummary({ terms }: { terms: ExamTerm[] }) {
        ...
        <div className="flex flex-col gap-2 mt-2.5">
            <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5 overflow-hidden">
                    {terms.slice(0, 3).map((t) => <div key={t.id} className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${t.full || (t.capacity && t.capacity.occupied >= t.capacity.total) ? 'bg-base-200 text-base-content/30' : 'bg-primary/5 text-primary/70'}`}>{t.date.split('.').slice(0, 2).join('.')}</div>)}
