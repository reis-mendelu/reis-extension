...
export function ExamSectionCard({
  ...
  const subjectName =
    language === 'en' && subject.nameEn ? subject.nameEn : subject.nameCs || subject.name;
  const sectionName =
    language === 'en' && section.nameEn ? section.nameEn : section.nameCs || section.name;
    ...
    >
      ...
      >
        <div className="flex-1 min-w-[200px]">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {isReg && (
              <span className="badge badge-success badge-outline badge-sm font-semibold">
                {t('exams.registered')}
              </span>
            )}
          ...
          {isReg && (
            ...
            >
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (!isAfterDeadline) onUnregister(section);
                }}
      ...
      {(section.registeredTerm?.id || section.terms[0]?.id) && (
        <div className={!isExpanded ? 'hidden md:block' : ''}>
          <TermNoteBlock
            terminId={(section.registeredTerm?.id ?? section.terms[0]?.id) as string}
          />
        </div>
      )}
...
function SectionStatePill({
  ...
  state: ReturnType<typeof getSectionState>;
  t: (k: string) => string;
}) {
  if (state.type === 'open')
    return (
