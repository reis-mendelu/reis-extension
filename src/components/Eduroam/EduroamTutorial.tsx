...
import { EDUROAM_MANUAL, manualKey, type EduroamAction, type DesktopEduroamTarget } from './manual';
import type { EduroamStatus } from '../../hooks/data/useEduroamSetup';

interface EduroamTutorialProps {
  target: DesktopEduroamTarget;
  ...
  return (
    <div className="flex gap-3.5 mb-4">
      <div className="flex flex-col items-center shrink-0">
        <span
          className={`flex items-center justify-center w-7 h-7 rounded-full font-bold text-sm ${done ? 'bg-success text-success-content' : 'bg-base-100 border border-base-content/10'}`}
        >
          {done ? <Check className="w-4 h-4" /> : n}
        </span>
        {!last && (
          <div className="w-px flex-1 bg-gradient-to-b from-base-content/15 to-transparent mt-1 min-h-2" />
        )}
      </div>
...
export function EduroamTutorial({
  target,
  status,
  password,
  onRun,
  onOpenSettings,
}: EduroamTutorialProps) {
  const { t } = useTranslation();
        ...
        return (
          <NumberedRow key={i} n={i + 1 + (manual.doOnceUrl ? 1 : 0)} last={false}>
            <div className="text-base leading-relaxed">
              {t(manualKey(target, 'steps', i, 'text'))}
            </div>
