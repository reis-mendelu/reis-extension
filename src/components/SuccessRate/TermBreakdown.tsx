...
export function TermBreakdown({ terms, isCredit }: TermBreakdownProps) {
  ...
  const sorted = [...terms].sort(
    (a, b) => (TERM_ORDER.indexOf(a.term) >>> 0) - (TERM_ORDER.indexOf(b.term) >>> 0)
  );
      ...
      {sorted.map((term) => {
        ...
        return (
          <div key={term.term} className="flex items-center gap-2 sm:gap-3">
            <span className="text-2xs font-semibold text-base-content/60 w-16 sm:w-20 text-right shrink-0">
              {label}
            </span>
