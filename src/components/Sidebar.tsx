  ...
  return (
    <aside className="flex touch:hidden flex-col w-20 h-screen bg-base-200 border-r border-base-300 items-center py-6 shrink-0 relative z-40">
      ...
      <div className="flex flex-col gap-3 w-full px-2">
        {menuItems.map(item => (
          <NavItem key={item.id} item={item} isActive={(currentView === 'exams' && item.id === 'exams') || (currentView === 'calendar' && item.id === 'dashboard') || (currentView === 'subjects' && item.id === 'subjects') || (currentView === 'erasmus' && item.id === 'erasmus')} isHovered={hovered === item.id} onMouseEnter={() => handleEnter(item.id)} onMouseLeave={handleLeave} onClick={() => { if (item.id === 'dashboard') onViewChange('calendar'); else if (item.id === 'exams') onViewChange('exams'); else if (item.id === 'subjects') onViewChange('subjects'); else if (item.id === 'erasmus') onViewChange('erasmus'); else if (item.href) window.open(item.href, '_blank'); }} onViewChange={onViewChange} onOpenSubject={onOpenSubject} />
        ))}
      </div>
