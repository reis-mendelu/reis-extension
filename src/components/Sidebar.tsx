...
export const Sidebar = ({
  ...
  const handleLeave = () => {
    timeout.current = setTimeout(() => setHovered(null), 300);
  };
        ...
        {menuItems.map((item) => {
          ...
          if (item.type === 'header') {
            return (
              <div
                key={item.id}
                className="mt-4 mb-1 text-[10px] font-bold uppercase tracking-wider text-base-content/30 w-full text-center px-1 truncate"
              >
                {item.label}
              </div>
            );
