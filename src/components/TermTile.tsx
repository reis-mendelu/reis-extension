...
export function TermTile({ term, onSelect, isProcessing = false }: { term: ExamTerm; onSelect: () => void; isProcessing?: boolean }) {
    ...
    return (
        <button onClick={() => !disabled && onSelect()} disabled={disabled}
            ...
            <div className="flex flex-col min-w-[80px]"><span className={`font-semibold ${disabled ? 'text-base-content/50 line-through' : ''}`}>{term.date.split('.').slice(0, 2).join('.')}</span><span className="text-xs text-base-content/60">{getDayOfWeek(term.date)}</span></div>
            ...
            {term.room && <div className="flex items-center gap-1 flex-1 min-w-0"><MapPin size={12} className="text-base-content/40 shrink-0" /><span className="text-sm truncate opacity-70">{term.room}</span></div>}
            {isFuture ? <div className="flex items-center gap-2 min-w-[120px] bg-warning/10 px-2 py-1 rounded-md"><Timer size={14} className="text-warning" /><div className="flex flex-col"><span className="text-xs font-medium text-warning">Za {formatCountdown(regStart!.getTime() - now.getTime())}</span><span className="text-[10px] text-warning/70">{term.registrationStart?.split(' ')[0]}</span></div></div>
            : term.capacity && <div className="flex items-center gap-2 min-w-[90px]"><Users size={12} className="text-base-content/40" /><div className="flex items-center gap-1.5"><progress className={`progress w-12 h-1.5 ${disabled ? 'progress-error' : 'progress-primary'}`} value={Math.min(100, (term.capacity.occupied / term.capacity.total) * 100)} max="100" /><span className={`text-xs ${disabled ? 'text-error font-medium' : 'opacity-50'}`}>{isFull ? 'PLNÝ' : (isClosed || isBlocked) ? 'UZAVŘENO' : term.capacity.raw}</span></div></div> }
            <div className="shrink-0 ml-auto">{isProcessing ? <span className="loading loading-spinner loading-sm text-primary"></span> : isFuture ? <span className="text-warning text-sm">⏳</span> : (isFull || isClosed || isBlocked) ? <span className="text-error/60 text-sm font-medium">✕</span> : <span className="btn btn-primary btn-sm">Přihlásit se</span>}</div>
        </button>
    );
}
