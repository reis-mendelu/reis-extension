...
export function TermTile({ term, onSelect, isProcessing = false }: { term: ExamTerm; onSelect: () => void; isProcessing?: boolean }) {
    ...
    return (
        <button onClick={() => !disabled && onSelect()} disabled={disabled}
                className={`flex flex-col w-full rounded-lg border transition-all text-left ${isFuture ? 'bg-warning/5 border-warning/30' : (isFull || isClosed || isBlocked) ? 'bg-base-200 opacity-60' : 'bg-base-100 hover:border-primary shadow-sm'}`}>
