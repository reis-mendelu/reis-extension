...
const COLOCATED = new Map<number, string[]>(
  LANDMARKS.map((l) => {
    ...
    const others = LANDMARKS.filter((o) => o.id !== l.id && GROUP_LABELS.get(o.id) === label).map(
      (o) => o.name
    );
    return [l.id, others];
  })
...
export function DetailPanel() {
  ...
  return (
    <div className="p-4 bg-base-100 border border-base-300 rounded-lg space-y-1">
      ...
      {r?.hasProjector && (
        <span className="badge badge-sm badge-info mr-1">{t('map.projector')}</span>
      )}
