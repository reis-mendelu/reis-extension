...
function remotePlaceExtent(place: RemotePlace): {
  minX: number;
  minY: number;
  maxX: number;
  maxY: number;
...
export function searchPlaces(
  ...
  const places = pois
    ...
    .map((f) => ({
      sel: { kind: 'poi', poi: f.properties, coord: f.geometry.coordinates } as MapSelection,
      rank: matchRank(q, f.properties.name, ''),
    }));
