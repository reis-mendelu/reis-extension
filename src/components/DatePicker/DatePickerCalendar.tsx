    ...
    <div className="p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-0.5">
          <button
            ...
            className="w-6 h-6 flex items-center justify-center hover:bg-base-200 rounded text-base-content/60"
          >
            <ChevronLeft size={14} />
          </button>
          <button
      ...
      <div className="grid grid-cols-7 gap-0">
        {daysOfWeek.map((d, i) => (
          <div
            key={i}
            className="h-6 flex items-center justify-center text-[10px] font-medium text-base-content/50 uppercase"
          >
            {d}
          </div>
        ))}
          ...
          <div key={wi} className="grid grid-cols-7">
            {week.map((day) => {
              ...
              return (
                <div
                  key={day.toString()}
                  className={`flex flex-col items-center justify-start relative ${other ? 'h-12' : 'h-8'}`}
                >
                  <button
                    onClick={() => hasAvailable && onSelectDate(dateInfo.dateStr)}
