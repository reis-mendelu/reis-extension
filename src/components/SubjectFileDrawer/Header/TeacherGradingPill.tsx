...
export function TeacherGradingPill({ teacherId }: { teacherId: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    async function init() {
      const sid = await getOrCreateVoteId(teacherId);
  ...
  async function remove() {
    ...
    const { error } = await supabase.rpc('delete_subject_rating', {
  ...
  if (winners.length > 0) {
    return (
      <button onClick={() => setOpen(true)} className="inline-flex items-center gap-1 group">
        <span
          className={`w-1.5 h-1.5 rounded-full ${DOT[winners[0]]} shrink-0 opacity-70 group-hover:opacity-100 transition-opacity`}
        />
        ...
        <span className="text-[10px] text-base-content/20 group-hover:text-base-content/35 transition-colors">
          {total}
        </span>
      </button>
    );
