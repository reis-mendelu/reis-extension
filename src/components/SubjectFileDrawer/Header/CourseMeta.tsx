        ...
        <div className="flex flex-col gap-2 w-full mt-1 text-sm text-base-content/60">
            ...
            {(courseInfo?.teachers?.length ?? 0) > 0 && (
                <div className="flex items-start gap-2"><span className="text-[13px] text-base-content/40 italic font-bold mt-0.5">{t('course.teachers')}</span>
                    <div className="flex flex-col gap-1.5 flex-1">{displayed?.map((t, i) => <div key={i} className="flex items-center gap-2 leading-none"><span className="text-[13px] font-bold text-base-content/70">{t.name}</span>{t.roles && <span className="text-[11px] text-base-content/40">({t.roles.toLowerCase()})</span>}</div>)}
