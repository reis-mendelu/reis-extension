...
export function CourseMeta({ lesson, courseInfo, isSearchContext }: any) {
    ...
    return (
        <div className="flex flex-col gap-2 w-full mt-1 text-sm text-base-content/60">
            {courseInfo?.garant && <div className="flex items-center gap-2"><User size={14} className="text-base-content/30" /><span className="text-[13px] text-base-content/40 italic font-bold">{t('course.garant')} <span className="font-bold text-base-content/70 not-italic">{courseInfo.garant}</span></span></div>}
            {courseInfo?.teachers?.length > 0 && (
                <div className="flex items-start gap-2"><span className="text-[13px] text-base-content/40 italic font-bold mt-0.5">{t('course.teachers')}</span>
                    <div className="flex flex-col gap-1.5 flex-1">{displayed?.map((t: any, i: number) => <div key={i} className="flex items-center gap-2 leading-none"><span className="text-[13px] font-bold text-base-content/70">{t.name}</span>{t.roles && <span className="text-[11px] text-base-content/40">({t.roles.toLowerCase()})</span>}</div>)}
                        {courseInfo.teachers.length > 3 && <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-1 text-[11px] font-bold text-primary">{expanded ? <><span>{t('course.showLess')}</span><ChevronUp size={12} /></> : <><span>{t('course.showMore')} ({courseInfo.teachers.length - 3})</span><ChevronDown size={12} /></>}</button>}
                    </div>
                </div>
            )}
        </div>
