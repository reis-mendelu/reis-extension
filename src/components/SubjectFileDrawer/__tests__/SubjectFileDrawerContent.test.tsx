...
vi.mock('../FileList', () => ({
    FileList: (props: { courseCode: string; groups: unknown[] }) => (
        <div data-testid="file-list" data-course-code={props.courseCode} data-groups={props.groups.length} />
    ),
...
vi.mock('../../SuccessRateTab', () => ({
    SuccessRateTab: (props: { courseCode: string; facultyCode?: string }) => (
        <div data-testid="success-rate-tab" data-course-code={props.courseCode} data-faculty-code={props.facultyCode} />
    ),
}));
...
const groupedFiles: FileGroup[] = [
    { name: 'root', displayName: 'root', files: [] },
];
...
describe('SubjectFileDrawerContent — files tab', () => {
    ...
    it('shows the empty state when not loading/syncing and there are no files', () => {
        render(
            <SubjectFileDrawerContent
                {...baseProps({ isFilesLoading: false, isSyncing: false, files: null, folderUrl: 'https://is.mendelu.cz/folder' })}
            />
        );
    ...
    it('renders FileList (not the empty state) once syncing starts, even while files are still empty', () => {
        ...
        render(<SubjectFileDrawerContent {...baseProps({ isFilesLoading: false, isSyncing: true, files: null })} />);
    ...
    it('renders FileList with the grouped files once files are present', () => {
        render(
            <SubjectFileDrawerContent
                {...baseProps({ files: [{ subfolder: '', file_name: 'a', file_comment: '', author: '', date: '', files: [] }] })}
            />
        );
...
describe('SubjectFileDrawerContent — tab switching', () => {
    ...
    it('swaps the rendered body when activeTab changes, unmounting the previous one', () => {
        const { rerender } = render(<SubjectFileDrawerContent {...baseProps({ activeTab: 'files' })} />);
