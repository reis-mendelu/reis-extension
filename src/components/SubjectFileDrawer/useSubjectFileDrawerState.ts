...
export function useSubjectFileDrawerState(lesson: BlockLesson | SelectedSubject | null, isOpen: boolean) {
    ...
    const _isFromSearch = lesson && 'isFromSearch' in lesson ? lesson.isFromSearch : false;
    const [activeTab, setActiveTab] = useState<'files' | 'stats' | 'assessments' | 'syllabus' | 'classmates' | 'cvicneTests'>(requestedTab ?? (isExam || !isEnrolled ? 'stats' : 'files'));
