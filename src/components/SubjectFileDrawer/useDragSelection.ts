...
export function useDragSelection({
  ...
  const draggingRef = useRef(false),
    ...
    initialIds = useRef<string[]>([]),
    ignoreRef = useRef(false);
  ...
  const onUp = useCallback(() => {
    ...
    if (draggingRef.current) {
      ...
      setTimeout(() => (ignoreRef.current = false), 100);
      setDragging(false);
      draggingRef.current = false;
    }
    setStart(null);
  ...
  return {
    selectedIds: ids,
    isDragging: dragging,
    ignoreClickRef: ignoreRef,
    setSelectedIds: setIds,
    clearSelection: () => setIds([]),
    selectionBoxStyle: useMemo(
      () =>
        !start || !end
          ? null
          : {
    ...
    handleMouseDown: (e: React.MouseEvent) => {
      ...
      const x = e.clientX - r.left + containerRef.current.scrollLeft,
        y = e.clientY - r.top + containerRef.current.scrollTop;
      setStart({ x, y });
      setEnd({ x, y });
    ...
    toggleSelect: (id: string, e: React.SyntheticEvent) => {
      e.stopPropagation();
