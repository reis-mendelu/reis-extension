...
export function useSelectionProcessor(
  ...
  return useCallback(
    (clientX: number, clientY: number) => {
      ...
      const cx = Math.max(0, Math.min(x, cw)),
        cy = Math.max(0, Math.min(y, ch));
      setSelectionEnd({ x: cx, y: cy });
      const bl = Math.min(selectionStartRef.current.x, cx),
        bt = Math.min(selectionStartRef.current.y, cy);
      const br = Math.max(selectionStartRef.current.x, cx),
        bb = Math.max(selectionStartRef.current.y, cy);
