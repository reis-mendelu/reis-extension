  ...
  return (
    <div className="bg-base-200/60 rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 gap-2">
        <span className="text-[12px] font-semibold text-base-content/80 flex-1 min-w-0 truncate">
          {name}
        </span>
        <div className="flex items-center gap-2 shrink-0">
          ...
          {showToggle && (
            ...
            >
              {attempts.length}×{expanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
            </button>
          )}
        </div>
      </div>
        ...
        <div className="border-t border-base-300 divide-y divide-base-300">
          {attempts.map((a, i) => (
            <div key={i} className="flex items-center gap-2 px-3 py-1.5">
              ...
              {a.teacher && (
                <span className="text-[11px] text-base-content/40 ml-auto truncate max-w-[120px]">
                  {a.teacher}
                </span>
              )}
            </div>
