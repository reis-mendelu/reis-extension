...
export function NoteCard({ card, autoFocus, onChange, onEnterAnswer, onDelete }: NoteCardProps) {
  ...
  const removeImage = (hash: string) =>
    onChange((c) => ({ images: c.images.filter((h) => h !== hash) }));
    ...
    <div className="group rounded-lg border border-base-300 bg-base-200/30 px-2 py-1.5">
      <div className="flex items-center gap-1.5">
        ...
        >
          <ChevronRight
            size={14}
            className={`transition-transform duration-200 ${!card.collapsed ? 'rotate-90 text-primary' : ''}`}
          />
        </button>
        <input
          ref={questionRef}
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {card.images.map((h) => (
                <Thumb
                  key={h}
                  hash={h}
                  onRemove={() => removeImage(h)}
                  removeLabel={t('course.documentNote.removeImage')}
                />
              ))}
            </div>
          )}
