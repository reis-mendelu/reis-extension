...
/**
 * Rooms the floor plan draws in pieces, and the one room each really is.
 *
 ...
 * had no polygon to point at. The two partitions below X01 are the MENDELU Shop.
 *
 ...
 * N1001/N1002/N1003/N1004. Each merged room keeps a real member's code and
 ...
 * union and should not grow one for four rooms.
 */
...
function mergedFeature(room: MergedRoom, template: RoomFeature): RoomFeature {
  return {
    type: 'Feature',
    geometry: { type: 'Polygon', coordinates: [room.ring] },
    properties: {
      ...template.properties,
      id: room.id,
      name: room.name,
...
export function mergeRoomGroups(rooms: RoomsCollection): RoomsCollection {
  ...
  for (const room of MERGED_ROOMS) {
    ...
    const merged = mergedFeature(room, features[first]!);
