...
const subject = (
  code: string,
  registered?: { id?: string; date: string; time: string }
): ExamSubject => ({
  version: 1,
  id: code,
  name: code,
  ...
  sections: [
    {
      ...
      name: 'zkouška',
      type: 'exam',
      status: registered ? 'registered' : 'open',
...
describe('buildExamTimeline', () => {
  ...
  it('builds a point with days remaining', () => {
    const pts = buildExamTimeline(
      [subject('EBC-ALG', { id: 't1', date: '25.04.2026', time: '09:00' })],
      now
    );
  ...
  it('carries a day-and-month short label beside the full one', () => {
    const pts = buildExamTimeline(
      [subject('EBC-ALG', { id: 't1', date: '05.04.2026', time: '09:00' })],
      now
    );
  ...
  it('reports zero days left for a term today', () => {
    const pts = buildExamTimeline(
      [subject('EBC-ALG', { id: 't1', date: '20.04.2026', time: '14:00' })],
      now
    );
  ...
  it('skips malformed dates instead of throwing', () => {
    expect(
      buildExamTimeline([subject('BAD', { id: 'x', date: 'not-a-date', time: '09:00' })], now)
    ).toEqual([]);
  });
});
