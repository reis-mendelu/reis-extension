...
import {
    startOfWeek, isSameWeek, isSameDay, trimHour, formatDayMonth,
    formatWhenShort, formatWhenRow, splitByWeek,
} from '../examWhen';
...
describe('formatWhenShort', () => {
    it('says "dnes" instead of repeating today\'s date', () => {
        expect(formatWhenShort(new Date(2026, 6, 20), '15:00', monday, 'cs-CZ', 'dnes')).toBe('dnes · 15:00');
    });

    it('spells out any other day', () => {
        expect(formatWhenShort(new Date(2026, 6, 21), '08:00', monday, 'cs-CZ', 'dnes')).toBe('út 21. 7. · 8:00');
    });
});
...
describe('splitByWeek', () => {
    it('splits on the week boundary, not on a rolling seven days', () => {
        const dates = [
            new Date(2026, 6, 22), new Date(2026, 6, 24), new Date(2026, 6, 26),
            new Date(2026, 6, 27), new Date(2026, 6, 31),
        ];
    ...
    it('keeps an already-passed exam in this week', () => {
        const { thisWeek } = splitByWeek([new Date(2026, 6, 20, 8, 0)], (d) => d, new Date(2026, 6, 22, 9, 0));
