export const getCzechHoliday = (date: Date, lang: 'cz' | 'en' = 'cz'): string | null => {
  ...
  if (day === 5 && month === 7)
    return isEn ? 'Saints Cyril and Methodius Day' : 'Den slovanských věrozvěstů Cyrila a Metoděje';
  if (day === 6 && month === 7) return isEn ? 'Jan Hus Day' : 'Den upálení mistra Jana Husa';
  if (day === 28 && month === 9)
    return isEn ? 'St. Wenceslas Day (Czech Statehood Day)' : 'Den české státnosti';
  if (day === 28 && month === 10)
    return isEn
      ? 'Independent Czechoslovak State Day'
      : 'Den vzniku samostatného československého státu';
  if (day === 17 && month === 11)
    return isEn ? 'Struggle for Freedom and Democracy Day' : 'Den boje za svobodu a demokracii';
  ...
  if (date.getDate() === goodFriday.getDate() && date.getMonth() === goodFriday.getMonth())
    return isEn ? 'Good Friday' : 'Velký pátek';
  if (date.getDate() === easterMonday.getDate() && date.getMonth() === easterMonday.getMonth())
    return isEn ? 'Easter Monday' : 'Velikonoční pondělí';
