...
export function computeTargetDimensions(w: number, h: number, maxEdge: number): { w: number; h: number } {
 ...
 */
export async function normalizeImage(input: Blob): Promise<{ blob: Blob; mime: string; w: number; h: number }> {
