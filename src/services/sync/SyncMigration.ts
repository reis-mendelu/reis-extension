...
async function migrateKey(key: string, value: any): Promise<void> {
  try {
    if (key === STORAGE_KEYS.SCHEDULE_DATA)
      await IndexedDBService.set('schedule', 'current', value);
    // SCHEDULE_WEEK_START is deliberately not migrated: nothing reads it.
    else if (key === STORAGE_KEYS.EXAMS_DATA) await IndexedDBService.set('exams', 'current', value);
    else if (key === STORAGE_KEYS.SUBJECTS_DATA)
      await IndexedDBService.set('subjects', 'current', value);
    else if (key === STORAGE_KEYS.USER_PARAMS)
      await IndexedDBService.set('meta', STORAGE_KEYS.USER_PARAMS, value);
    else if (key === STORAGE_KEYS.STUDY_PROGRAM_DATA)
      await IndexedDBService.set('meta', 'study_program', value);
    else if (key === STORAGE_KEYS.LAST_SYNC) await IndexedDBService.set('meta', 'last_sync', value);
    else if (key === STORAGE_KEYS.SYNC_ERROR)
      await IndexedDBService.set('meta', 'sync_error', value);
    else if (key.startsWith(STORAGE_KEYS.SUBJECT_FILES_PREFIX))
      await IndexedDBService.set(
        'files',
        key.replace(STORAGE_KEYS.SUBJECT_FILES_PREFIX, ''),
        value
      );
    else if (key.startsWith(STORAGE_KEYS.SUBJECT_ASSESSMENTS_PREFIX))
      await IndexedDBService.set(
        'assessments',
        key.replace(STORAGE_KEYS.SUBJECT_ASSESSMENTS_PREFIX, ''),
        value
      );
    else if (key.startsWith(STORAGE_KEYS.SUBJECT_SYLLABUS_PREFIX))
      await IndexedDBService.set(
        'syllabuses',
        key.replace(STORAGE_KEYS.SUBJECT_SYLLABUS_PREFIX, ''),
        value
      );
    else if (key === STORAGE_KEYS.SUCCESS_RATES_DATA)
      await IndexedDBService.set('success_rates', 'current', value);
    else if (key === 'reis_calendar_click_hint_shown')
      await IndexedDBService.set('meta', 'calendar_click_hint_shown', value === 'true');
    else if (key === 'reis_drag_hint_shown')
      await IndexedDBService.set('meta', 'drag_hint_shown', value === 'true');
    else if (key === 'reis_welcome_dismissed')
      await IndexedDBService.set('meta', 'welcome_dismissed', value === 'true');
      ...
      await IndexedDBService.set(
        'meta',
        `assessment_adjustments_${key.replace('assessment-adjustments-', '')}`,
        value
      );
    else if (key.startsWith('bonus-points-'))
      await IndexedDBService.set('meta', `bonus_points_${key.replace('bonus-points-', '')}`, value);
    else if (key === 'user_id') await IndexedDBService.set('meta', 'user_id', value);
    else await IndexedDBService.set('meta', key, value);
  } catch {
    /* Individual key migration failure is non-critical */
  }
}
