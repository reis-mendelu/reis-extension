...
async function runNotesPass(
  ...
  for (const subject of subjects) {
    try {
      ...
      if (prev && prev.docHash === hash && (!hasImages || prev.imagesEmbedded)) {
        skipped++;
        continue;
      }
      ...
      if (docId) {
        ...
        docId = (
          await uploadDoc(subject.title, html, [subjectFolderId], { [DOC_PROP]: subject.code })
        ).id;
      }
      ...
      if (jsonId) {
        ...
        jsonId = (
          await uploadFile(`${subject.code}.json`, jsonBlob, [notesFolderId], {
            [JSON_PROP]: subject.code,
          })
        ).id;
      }
  ...
  for (const code of Object.keys(manifest.notes)) {
    ...
    try {
      // safe: code came from Object.keys(manifest.notes)
