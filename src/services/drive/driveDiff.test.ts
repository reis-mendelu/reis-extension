...
describe('flattenSubjectFiles', () => {
  it('puts a file with a subfolder under subject/subfolder', () => {
    const items = flattenSubjectFiles('BIK-DBS - Databases', [
      pf({
        subfolder: 'Lectures',
        date: '2026-03-01',
        files: [{ name: 'l1.pdf', type: 'pdf', link: 'L1' }],
      }),
    ]);
    expect(items).toEqual([
      {
        ...
        fileName: 'l1.pdf',
        date: '2026-03-01',
        pathSegments: ['BIK-DBS - Databases', 'Lectures'],
      },
    ]);
  });
  ...
  it('puts a file with no subfolder directly under the subject', () => {
    ...
    expect(items[0]!.pathSegments).toEqual(['SUB']); // safe: fixed literal, one item produced
  });
  ...
  it('emits one item per attachment in a group', () => {
    const items = flattenSubjectFiles('SUB', [
      pf({
        files: [
          { name: 'a', type: 'pdf', link: 'A' },
          { name: 'b', type: 'pdf', link: 'B' },
        ],
      }),
    ]);
  ...
  it('does not number a row that has a single attachment', () => {
    ...
    expect(items[0]!.fileName).toBe('dokument'); // safe: fixed literal, one item produced
  });
  ...
  it('folds the teacher comment into the Drive file name (teachers use it as the title)', () => {
    const items = flattenSubjectFiles('SUB', [
      pf({
        file_name: 'dokument',
        file_comment: 'Přednáška 5 – Normalizace',
        files: [{ name: 'dokument', type: 'pdf', link: 'L' }],
      }),
    ]);
    expect(items[0]!.fileName).toBe('dokument — Přednáška 5 – Normalizace'); // safe: fixed literal
  });
  ...
  it('keeps the bare name when there is no comment', () => {
    const items = flattenSubjectFiles('SUB', [
      pf({
        ...
        file_comment: '',
        files: [{ name: 'slides.pdf', type: 'pdf', link: 'L' }],
      }),
    ]);
    expect(items[0]!.fileName).toBe('slides.pdf'); // safe: fixed literal, one item produced
  });
});
...
describe('linkHash', () => {
  it('is a stable 24-char hex string', async () => {
    ...
    expect(
      await linkHash('https://is.mendelu.cz/auth/dok_server/slozka.pl?id=123;download=456')
    ).toBe(h);
  });
