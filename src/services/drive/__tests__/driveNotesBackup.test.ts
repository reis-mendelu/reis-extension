  ...
  files: [
    {
      fileLink: '/x',
      fileName: 'L1.pdf',
      note: JSON.stringify([{ id: 'a', type: 'text', content: 'hi' }]),
    },
  ],
};
...
describe('syncDriveNotesBackup', () => {
  ...
  it('creates a Doc + sidecar on first run and records them in the manifest', async () => {
    ...
    expect(manifest.notes['BIK-DBS']!.docId).toBe('doc1'); // safe: created by the sync above
  ...
  it('skips a subject with no non-empty notes (never writes empty)', async () => {
    const empty: SubjectNotes = {
      ...subject,
      files: [{ fileLink: '/y', fileName: 'E.pdf', note: '  ' }],
    };
    ...
    files: [
      {
        ...
        fileName: 'L1.pdf',
        note: JSON.stringify({
          cards: [{ id: 'a', question: 'q', answer: '', collapsed: false, images: ['hashAAA'] }],
          notes: '',
        }),
      },
    ],
  ...
  it('re-uploads when a subject with images was first stored text-only (override arrives later)', async () => {
    ...
    expect(drive.updateDocContent).toHaveBeenCalledWith(
      'doc1',
