...
describe('message schemas accept every real factory message', () => {
  ...
  it('accepts all Iframe→Content messages the factory can build', () => {
    const msgs = [
      ...
      // A LIVE action. This used to be 'download_file', which had no
      // handler case on any platform — so the one action this suite
      // exercised was the one that could never run.
      Messages.action('download_document', { url: 'https://is/x', filename: 'a.pdf' }),
...
describe('message schemas reject malformed input', () => {
  ...
  it('rejects wrong-shaped payloads for a known type', () => {
    ...
    expect(IframeToContentSchema.safeParse({ type: 'REIS_FETCH', id: 'x', url: 42 }).success).toBe(
      false
    );
    // REIS_POPUP_STATE with non-boolean open.
    expect(ContentToIframeSchema.safeParse({ type: 'REIS_POPUP_STATE', open: 'yes' }).success).toBe(
      false
    );
    // REIS_ACTION with an unknown action enum value.
    expect(
      IframeToContentSchema.safeParse({
        type: 'REIS_ACTION',
        id: 'x',
        action: 'rm_rf',
        payload: {},
