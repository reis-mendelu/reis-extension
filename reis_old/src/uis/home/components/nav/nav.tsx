...
import { OUTLOOK_IMAGE, TEAMS_IMAGE, type GenericProps } from "../variables";
...
function ReportNav(_: NavProps) {
    return (
        <nav className="relative w-full h-16 bg-gray-50 shadow-xl p-2 font-dm select-none pl-4 flex flex-row items-center">
            <button onClick={_.onToggleSidebar} className="p-1 hover:bg-gray-200 rounded-full transition-colors">
...
function SchedueleNav(_: NavProps) {
    return (
        <nav className="relative w-full h-16 bg-gray-50 shadow-xl p-2 font-dm select-none pl-4 flex flex-row items-center">
            ...
            {/*Tabs*/}
            <div className="hidden h-full min-w-64 w-fit md:flex flex-row items-center [&>*]:mr-5 ml-[15px]">
...
export function Nav(_: NavProps) {
    if (_.page == undefined || _.page == "home") {
        ...
        switch (_.page.toLowerCase()) {
            case "report":
                return <ReportNav setScreen={_.setScreen} onToggleSidebar={_.onToggleSidebar} />
            case "scheduele":
                return <SchedueleNav setScreen={_.setScreen} onToggleSidebar={_.onToggleSidebar} />
        }
    }
